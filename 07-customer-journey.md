...
**Práctica añadida (riesgo de pérdida de clientes, retorno alto)**: comprometer una fecha de despliegue en el kickoff (Etapa 9) para que esta etapa no quede abierta sin límite, y considerar una regla de aceptación tácita (si el cliente no objeta en N días desde la publicación, se considera aceptado y Motor B arranca igualmente) — ambas usan herramientas ya existentes (el propio contrato, un calendario). **Actualizado 2026-07-12**: `contrato-minimo-venta.md` (sección 3) ya recoge este mismo disparador por escrito — la facturación de Motor B arranca en la conformidad o en la aceptación tácita tras el plazo pactado, no en la fecha de entrega. No son dos reglas independientes; el contrato es lo que el cliente firma, esta etapa describe cuándo se activa en la práctica.
