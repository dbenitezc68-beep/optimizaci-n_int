...
El contrato (`contrato-minimo-venta.md`) está listo como plantilla. *(Actualizado 2026-07-15: la duración mínima de empaquetado de Motor B ya no es un campo en blanco — ADR-002 la fija en 6 meses, ver `decisiones-adr.md`.)* **La firma real de un contrato vinculante sigue bloqueada hoy** por el único campo que queda en blanco: identificación fiscal de INTEREMPREX (razón social/NIF/domicilio, L8) — confirmado que INTEREMPREX no está dado de alta todavía como autónomo/sociedad, con alta planificada para septiembre de 2026 (ver `priorizacion.md` y `alta-autonomo.md`). No es trabajo de Fase 7 — es una decisión ya tomada por el fundador, ejecutándose por calendario, no por documentación pendiente. Dado que la tarifa de lanzamiento cierra el 2026-09-30 (o al tercer cliente, lo que ocurra antes) y hoy ya hay 2 pilotos, esta sigue siendo la pieza más urgente de todo el documento.
...
**Riesgos**: (1) sin plantilla, inconsistencia entre propuestas; (2) **riesgo crítico ya corregido (2026-07-15)**: la auditoría adversarial había encontrado que un cliente podía llegar a aceptar (Etapa 6) sin conocer la duración de empaquetado de Motor B, porque ese número solo aparecía por primera vez en el contrato (Etapa 7). **Ya no aplica: ADR-002 fija la duración mínima en 6 meses de permanencia inicial** (`decisiones-adr.md`) — la Propuesta debe comunicar esta cifra ya cerrada, no una condición a concretar después; (3) igual que en la Etapa 4, la exclusión de Motor C y *toda* la línea Gestión del crecimiento (incluido Nivel 1) debe quedar fuera de la propuesta; (4) sin plazo de validez definido; (5) dependencia total del fundador.
...
**Riesgos**: *(actualizado 2026-07-15)* de los 2 campos en blanco que bloqueaban una firma real y completa, **queda uno**: razón social/NIF/domicilio de INTEREMPREX, no confirmado porque INTEREMPREX no está dado de alta todavía como autónomo/sociedad — alta planificada para septiembre de 2026, no ejecutada (ver `priorizacion.md`, `alta-autonomo.md`). Sin esto no se puede identificar legalmente a una de las partes. La duración mínima de Motor B **ya no bloquea**: ADR-002 la fija en 6 meses (`decisiones-adr.md`), y el plazo de aceptación tácita (sección 3 del contrato) queda fijado por ADR-003 en 7 días naturales. Firmar hoy con el campo fiscal en blanco expondría a INTEREMPREX a un contrato sin identificar correctamente a una de las partes. Riesgos menores: DPA con Stripe no confirmado (L5); aviso legal completo de la web no confirmado (L8).
**KPIs**: sin dato — candidatos: tiempo negociación→firma, % de negociaciones que llegan a contrato firmado.
**Puntos de decisión**: forma de pago concreta (sección 3, sigue en blanco — no cubierta por ningún ADR todavía); reconfirmar criterios de aceptación.
**¿Ejecutable mañana?** **NO**, con precisión: la *plantilla* sí está lista hoy, con la duración de Motor B (ADR-002) y el plazo de aceptación tácita (ADR-003) ya resueltos. La *firma real* sigue bloqueada por el único campo que queda en blanco — la identificación fiscal de INTEREMPREX — porque el alta como autónomo está planificada para septiembre de 2026, no ejecutada todavía.
**Hallazgo de la auditoría adversarial (cuellos de botella, retorno alto) — actualizado 2026-07-15**: de los 2 campos originales, 1 ya tiene fecha de resolución concreta (alta de autónomo, septiembre 2026) en vez de quedar sin plazo. La tarifa de lanzamiento cierra el 2026-09-30 (o al 3er cliente) — con 2 pilotos ya existentes, si un tercer lead llega a acuerdo antes de que el alta esté completada, el negocio sigue sin poder firmar, cobrar, ni activar Motor B con ese cliente. El riesgo de "sin fecha límite autoimpuesta" queda mitigado, no eliminado — ver `priorizacion.md` para el seguimiento.
**Diseño 1-10 clientes**: proceso manual, una vez resuelto el campo fiscal pendiente — el fundador adapta la plantilla, envía, gestiona la firma, actualiza el CRM a mano; revisión individual por cliente es apropiada a este volumen.
...
**Práctica añadida (riesgo de pérdida de clientes, retorno alto)**: comprometer una fecha de despliegue en el kickoff (Etapa 9) para que esta etapa no quede abierta sin límite, y considerar una regla de aceptación tácita — ambas usan herramientas ya existentes (el propio contrato, un calendario). **Actualizado 2026-07-12**: `contrato-minimo-venta.md` (sección 3) ya recoge este mismo disparador por escrito — la facturación de Motor B arranca en la conformidad o en la aceptación tácita tras el plazo pactado, no en la fecha de entrega. No son dos reglas independientes; el contrato es lo que el cliente firma, esta etapa describe cuándo se activa en la práctica. **Actualizado 2026-07-15**: el plazo ya no es una N genérica — **7 días naturales desde la notificación de entrega, con recordatorio escrito al día 5 y observaciones válidas solo por el canal pactado en el contrato** (ADR-003, `decisiones-adr.md`). La duración mínima de Motor B tampoco es una incógnita: **6 meses de permanencia inicial, prórroga automática mensual, preaviso de baja 30 días por escrito** (ADR-002, `decisiones-adr.md`) — ver `contrato-minimo-venta.md`, secciones 3 y 4.
...
1. ~~Resolver los 2 campos en blanco del contrato antes del siguiente lead que llegue a Negociación~~ — **actualizado 2026-07-15**: la duración de empaquetado de Motor B ya está resuelta (ADR-002). Queda la identificación fiscal de INTEREMPREX, con alta como autónomo planificada para septiembre de 2026 (ver `priorizacion.md`, `alta-autonomo.md`) — sigue siendo la tarea de mayor urgencia de todo este documento.
...
1. ~~Duración mínima del periodo obligatorio de Motor B~~ — **Resuelta el 2026-07-15**: ADR-002 la fija en 6 meses de permanencia inicial, prórroga automática mensual, preaviso 30 días (`decisiones-adr.md`). Aplicada en `contrato-minimo-venta.md` y en la Etapa 12 de este documento.
2. **Identificación fiscal de INTEREMPREX** (razón social/NIF/domicilio) — sigue pendiente. Verificado: INTEREMPREX no está dado de alta todavía; alta como autónomo planificada para septiembre de 2026 (ver `priorizacion.md`, `alta-autonomo.md`). El NIF del contrato queda como campo a rellenar en el momento del alta, no antes.
