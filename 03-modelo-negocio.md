...
6. **R13 — Incidencia crítica de continuidad de negocio. `interemprex-dashboard` no tenía repositorio remoto en GitHub — existía únicamente en la máquina local.** *(Resuelto el 2026-07-07 — repositorio privado creado, remoto configurado, push confirmado; detalle en `production-readiness-review.md` y `priorizacion.md`. Se deja la descripción original del riesgo sin reescribir, por ser una fase cerrada, y se añade esta nota de estado.)* Era el sistema que sostiene el motor de ingresos real (Motor B: clientes, pipeline, pagos vía Stripe, MRR). Si esa máquina se hubiera perdido, corrompido o quedado inaccesible, se habría perdido el código completo del CRM sin ninguna copia externa — no era un riesgo hipotético de "algún día", era el estado real verificado en `inventario-tecnologico.md`. Se relaciona con R12 (mismo origen: dependencia total de la máquina/persona del fundador) pero es un riesgo distinto: R12 es sobre *quién* mantiene el sistema, R13 era sobre *dónde* existía el sistema. Ver procedimiento más abajo (ya ejecutado, ver nota de estado).
...
8. **R6 — Tecnológico/operativo. Sin evidencia de estrategia de backup o continuidad para el dashboard.** *(Resuelto el 2026-07-09 — sistema de backup automático, diario y verificado, implementado; detalle completo en `production-readiness-review.md`. Se deja la descripción original sin reescribir, por ser una fase cerrada, y se añade esta nota de estado.)* En el momento de esta fase no se había encontrado documentación de copias de seguridad automatizadas para los datos de clientes, pipeline y pagos — a diferencia del panel de cumplimiento (`gestion-interemprex`), que sí tenía una copia de seguridad simple documentada.
...
### R13 — Procedimiento (ya ejecutado, ver nota de estado arriba)
...
**Qué documentos deben revisarse si este cambia**: `04-arquitectura-oferta.md` de inmediato; el resto de fases futuras en cuanto existan, porque todas heredan la arquitectura de ingresos. Actualizado 2026-07-12: se eliminó una duplicación exacta de la sección de procedimiento de R13, se renumeró la lista de riesgos (numeración correlativa 1-13, antes tenía dos elementos "6"), y se añadieron notas de estado fechadas para R13 (resuelto 2026-07-07) y R6 (resuelto 2026-07-09) sin reescribir la descripción original de ningún riesgo — ambos ya ejecutados, ver `production-readiness-review.md`.
