...
- **Motor de atención y recomendación** (`src/lib/attention.ts`): consolida eventos de todos los módulos en una lista priorizada donde cada elemento explica qué pasa, por qué importa y **cuál es la siguiente acción recomendada** — incidencias abiertas, suscripciones impagadas, facturas vencidas, tareas vencidas o de hoy, renovaciones próximas, cobros manuales sin llegar, procesos retrasados/pausados/inactivos, leads sin movimiento, clientes activos sin seguimiento y **oportunidades de ampliación** (proyecto entregado sin ingreso recurrente detrás). Se muestra global en el Resumen ("Requiere tu atención") y contextual en el expediente de cada cliente ("Recomendaciones para este cliente"). Umbrales como constantes en un único sitio.
